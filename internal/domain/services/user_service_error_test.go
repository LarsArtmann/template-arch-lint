...
func (r *FailingUserRepository) FindByID(
	ctx context.Context,
	id values.UserID,
) (*entities.User, error) {
	r.findByIDCallCount++
...
func (r *FailingUserRepository) FindByEmail(
	ctx context.Context,
	email string,
) (*entities.User, error) {
...
func (r *FailingUserRepository) FindByUsername(
	ctx context.Context,
	username string,
) (*entities.User, error) {
	...
	Describe("🔥 Repository Error Propagation", func() {
		...
		Context("UpdateUser with repository failures", func() {
			It("should handle FindByID errors during update", func() {
				...
				Expect(user).To(BeNil())
				expectInternalErrorWithCause(err, sql.ErrConnDone, "failed to get user")
				Expect(failingRepo.findByIDCallCount).To(Equal(1))
			...
			It("should handle Update repository errors", func() {
				...
				testUser, _ := entities.NewUser(
					createTestUserID("test-user"),
					"test@example.com",
					"Test User",
				)
		...
		Context("with invalid user data", func() {
			It("should return proper validation errors for invalid email formats", func() {
				...
				for _, email := range invalidEmails {
					...
					Expect(
						isValidationError,
					).To(BeTrue(), "should be validation error for email: %s", email)
				}
			})
			...
			It("should return proper validation errors for invalid names", func() {
				...
				for _, name := range invalidNames {
					...
					Expect(
						isValidationError,
					).To(BeTrue(), "should be validation error for name: %s", name)
				}
			})
		})
	...
	Describe("📊 Error Recovery and Resilience", func() {
		Context("partial failure scenarios", func() {
			It("should maintain consistency during partial failures", func() {
				...
				Expect(
					err.Error(),
				).To(ContainSubstring("failed to get user"))
				Expect(err.Error()).To(ContainSubstring("sql: connection is already closed"))
